...
void init_items() {
	...
	for (size_t i = 0; i < json.child_count; i++) {
		...
		if (streq_nocase(tmp->data.string, "none")) ii->toolType = TOOL_NONE;
		else if (streq_nocase(tmp->data.string, "pickaxe")) ii->toolType = TOOL_PICKAXE;
		else if (streq_nocase(tmp->data.string, "shovel")) ii->toolType = TOOL_SHOVEL;
		else if (streq_nocase(tmp->data.string, "axe")) ii->toolType = TOOL_AXE;
		else if (streq_nocase(tmp->data.string, "hoe")) ii->toolType = TOOL_HOE;
		else if (streq_nocase(tmp->data.string, "sword")) ii->toolType = TOOL_SWORD;
		else if (streq_nocase(tmp->data.string, "shears")) ii->toolType = TOOL_SHEARS;
		else goto cerr;
